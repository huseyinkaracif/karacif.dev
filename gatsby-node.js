...
exports.createPages = async ({ graphql, actions }) => {
  ...
  Object.entries(bySlug).forEach(([slug, langs]) => {
    ;[`tr`, `en`].forEach((lang) => {
      ...
      // social card generated by scripts/generate-og-cards.js (committed to static/)
      const ogCard = `/images/og/${lang}-${slug}.png`
      const hasOgCard = fs.existsSync(path.join(__dirname, `static`, ogCard.slice(1)))
      createPage({
...
exports.onPostBuild = async ({ graphql }) => {
  ...
  for (const [lang, list] of [[`tr`, tr], [`en`, en]]) {
    const locale = lang === `tr` ? `tr-TR` : `en-US`
    const index = list.map((p) => ({
      t: p.frontmatter.title,
      u: ROUTES[lang].post(p.fields.postSlug),
      c: p.frontmatter.category || ``,
      e: p.frontmatter.excerpt || ``,
      d: p.frontmatter.date,
      r: p.frontmatter.readTime || ``,
      k: [(p.frontmatter.tags || []).join(` `), p.frontmatter.category || ``]
        .join(` `)
        .toLocaleLowerCase(locale),
      b: plainBody(p.rawMarkdownBody, locale),
    }))
    fs.writeFileSync(path.join(pub, `search-${lang}.json`), JSON.stringify(index), `utf-8`)
  }
}
