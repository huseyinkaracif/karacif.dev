...
export default function Controls({ lang = "tr", altPath }) {
  const { theme, toggleTheme } = useSite();
  const isDark = theme === "dark";

  const targets = {
    tr: lang === "tr" ? null : altPath || ROUTES.tr.home,
    en: lang === "en" ? null : altPath || ROUTES.en.home,
  };
  ...
  const langBtn = (l) => {
    const activeCls = "bg-primary-container text-on-primary-container";
    const idleCls = "text-on-surface-variant hover:text-on-surface";
    const cls = `px-2.5 py-1.5 rounded-lg text-[11px] font-black font-label tracking-widest uppercase transition-all duration-200 active:scale-90 ${lang === l ? activeCls : idleCls}`;
    if (!targets[l]) {
