...
export default function Projects({ data }) {
  ...
  return (
    <div className="bg-background font-body text-on-background antialiased selection:bg-primary-container selection:text-on-primary-container">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-xl shadow-[0_8px_32px_rgba(109,94,0,0.08)]">
        <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto">
          ...
          <div className="hidden md:flex items-center gap-8 font-headline font-bold tracking-tight">
            ...
            <a className="text-zinc-900 border-b-4 border-yellow-400 pb-1" href="/projects">Projeler</a>
            <a className="text-zinc-500 hover:text-zinc-900 transition-colors" href="/blog">Yazılar</a>
      ...
      {/* Location Badge */}
      <div className="fixed bottom-8 right-8 z-40 hidden lg:flex items-center gap-3 bg-white/70 backdrop-blur-md px-4 py-2 rounded-full border border-outline-variant/15 shadow-lg">
                ...
                {project.image ? (
                  <div className="relative overflow-hidden aspect-[4/3] shrink-0">
                    ...
                    <div className="absolute top-4 left-4 w-8 h-8 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center">
                      <span className="text-[11px] font-black font-headline text-zinc-900">{String(i + 1).padStart(2, "0")}</span>
                    </div>
                  </div>
                ) : (
                  <div className="relative aspect-[4/3] shrink-0 bg-surface-container-highest flex items-center justify-center">
                    <span className="text-5xl font-black font-headline text-outline-variant/30">{String(i + 1).padStart(2, "0")}</span>
                  </div>
                )}
      ...
      {/* Footer */}
      <footer className="bg-zinc-50 w-full py-8 md:py-12 px-6 pb-32 md:pb-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 max-w-7xl mx-auto">
          <div className="flex flex-col gap-2">
            ...
            <p className="font-['Inter'] text-sm tracking-wide text-zinc-500">© 2026 Hüseyin Karacif. Senior Software Developer.</p>
          </div>
          <div className="flex gap-8">
            <a className="text-[#0077B5] hover:text-[#005582] flex items-center gap-2 underline decoration-2 underline-offset-4 transition-all text-sm font-['Inter'] tracking-wide" href="https://www.linkedin.com/in/huseyin-karacif" target="_blank" rel="noopener noreferrer">
              <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-5 5 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
              LinkedIn
            </a>
            <a className="text-[#333] hover:text-gray-600 flex items-center gap-2 underline decoration-2 underline-offset-4 transition-all text-sm font-['Inter'] tracking-wide" href="https://github.com/huseyinkaracif" target="_blank" rel="noopener noreferrer">
